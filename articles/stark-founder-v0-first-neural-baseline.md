...
M7 performance measurements on the accepted local BF16 accelerator:
...
- canonical weights matched the expected frozen identity;
...
weights identity   frozen and independently verified
```
...
When Founder V0 was frozen, its weak context recall originally suggested a recurrent working-memory experiment. The subsequent design pass separated two different problems: temporary working context and **durable semantic factual knowledge**. M8 deliberately chose the latter so that the first post-Founder architecture would test whether Stark could gain knowledge without rewriting its language substrate.

M8 therefore became **Native Semantic Fact Memory** rather than a recurrent-context extension. It introduced SFM-1, a persistent neural semantic-memory state, exact factual evidence, governed consolidation, multi-memory recall, and a typed Global Workspace boundary to frozen Founder V0.

The central M8 result is now established: correct single-memory context usually moves Founder's target distribution in the right direction, but the weak V0 language substrate does not yet turn that signal into reliable exact answers. Memory storage and retrieval are no longer the primary bottleneck; selective cognitive utilization is.

## 17. Why this baseline still matters for Stark

The value of Founder V0 is not its raw benchmark score. It remains Stark's **known control organism**.

Every later architecture can still be asked:

```text
Did memory retrieval improve?
Did ordinary language loss regress?
Did an exact fact survive neural association?
Did READ accidentally become LEARN?
Did irrelevant memory interfere?
Can multiple memories coexist without memory itself reasoning?
Did the new mechanism alter the frozen Founder control?
Can the entire state survive a crash and deterministic restart?
```

That is why M8 could establish a semantic-memory organ without hiding Founder's limitations.
...
It learned genuine held-out language structure. It did not become a mature knowledge store, working-memory system, or general reasoning system. M8 then demonstrated that Stark can add a separate durable semantic-memory organ while preserving that control model exactly.

The project transition is now:

```text
M0–M7: build and freeze a measurable neural language substrate
M8:    add a native semantic-memory organ and prove causal integration
Next:  improve cognition/workspace utilization without weakening memory/truth boundaries
```

M9 is intentionally not defined by this article. Its architecture should be chosen only after its research question and acceptance criteria are frozen.
